...
export default function Learning() {
    ...
    const interval = setInterval(async () => {
      ...
      try {
        ...
        if (videoRef.current && typeof videoRef.current.currentTime !== "undefined") {
          ...
          totalSecs = videoRef.current.duration || 100;
        }
        ...
        await fetch("/api/history/update", {
          ...
          body: JSON.stringify({
            ...
            lessonId: learningData.currentLesson.id || 1,
            watchedSeconds: currentSecs,
            totalDuration: totalSecs,
          }),
        });

        console.log(`📡 Progress saved to DB: ${Math.round(currentSecs)} seconds`);
      } catch (error) {
  ...
  }, [learningData?.currentLesson, courseId, user]);
